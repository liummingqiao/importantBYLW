import LineStringStore from '../../store/line-string-store'
import GroupingOverlay from './grouping-overlay'
import mixin from '../../util/mixin'
import LineInteract from './line-interact'
...
class WebglLineOverlay extends mixin(GroupingOverlay, LineInteract) {
  /**
   * 元素被点中后触发
   ...
   * 仅支持 LineStringStore
   * @override
   * @param {LineStringStore} store
   */
  bindStore(store) {
    if (store.constructor !== LineStringStore) {
      return console.error('LineStringStore support only!')
    }
    const me = this
  ...
  _compileData() {
    ...
    Object.keys(groups).forEach(function(groupKey, idx) {
      ...
      groupData.forEach(function(row) {
        ...
        rawRow = store.fetchRaw(row)
        verts.push(pixel.x, pixel.y, color[0] / 255, color[1] / 255, color[2] / 255, aLegend.idx, rawRow._filtered === false ? 0.0 : 1.0)
      })
    })
    me.lastVerts = verts
  ...
  _storeFiltered(e) {
    ...
    if (verts && verts.length > 0) {
      ...
      Object.keys(groups).forEach(function(groupKey) {
        ...
        groupData.forEach(function(row) {
          rawRow = store.fetchRaw(row)
          verts[idx * 7 + 6] = rawRow._filtered === false ? 0.0 : 1.0
          idx++
        })
      })
    }
  ...
  onOpDraw(param) {
    ...
    ctx.lineWidth = me.options.highlightWidth

    me.lastClickedData.forEach(function(d) {
      if (d._filtered === false) {
        return
      }
      aLegend = legends[store.fetchRawGroup(d)]
      if (!aLegend.visible) {
        return
      }
      color = me.COLOR_SETTING.transform(aLegend.color)

      coords = turf.getCoords(d._turfData)
      ctx.beginPath()
      coords.forEach(function(ll, idx) {
        point = me.latLngToPoint(DataUtil.transform(ll[1], ll[0], store.crsCode, me.mapWrapper.getCrsCode()))
        if (idx === 0) {
  ...
  onDraw(param) {
    ...
    gl.drawArrays(gl.LINES, 0, store.getSize())
  }
  ...
  setData(data, storeCfg) {
    const store = new LineStringStore(data, storeCfg)
...
export default WebglLineOverlay
